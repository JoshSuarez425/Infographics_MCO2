...
local leaderstats = localPlayer2:WaitForChild("leaderstats", 10) or localPlayer2
...
local function setupCharacterHandlers(character)
 ...
 or not targetRoot2 or not targetRoot2.Parent then
		...
		if ultAnimIds[animId:match("%d+")] or animId:match("18896229321") then
			task.wait(0.01)
			...
			task.spawn(function()
				...
				local trackWaitDeadline = tick() + 2.2
				while not ultTrack and tick() < trackWaitDeadline do
					task.wait()
					...
					if target then
						...
						if myRoot2 and targetRoot2 then
							...
							if targetHumanoid then
								...
								task.spawn(function()
									local watchedVictim = target
									local dwellDeadline = tick() + 3
									while ultActive and currentTarget2 == watchedVictim do
										runService.Heartbeat:Wait()
										if not ultActive or currentTarget2 ~= watchedVictim then
											break
										end
										local watchedChar = getCharacter(watchedVictim)
										local watchedHumanoid = getHumanoidOf(watchedVictim)
										local watchedDead = not watchedHumanoid or watchedHumanoid.Health <= 0
										local watchedRagdolled = watchedChar
											and (watchedChar:FindFirstChild("Ragdoll") or watchedChar:FindFirstChild("RagdollSim"))
										local timedOut = tick() >= dwellDeadline
										if watchedDead or watchedRagdolled or timedOut then
											if doneTargets[watchedVictim] then
												break
											end
											doneTargets[watchedVictim] = true
											detachPhysics()
											local nextTarget = findNextTarget(watchedVictim)
											if nextTarget then
												setTarget(nextTarget)
												if findNextTarget(nextTarget) == nil and not anyCounteringPlayer() then
													task.wait(0.1)
				...
				local firstTargetDeadline = tick() + 1.5
				while not firstTarget and tick() < firstTargetDeadline do
					runService.Heartbeat:Wait()
					firstTarget = findNextTarget(nil)
				end
				if firstTarget then
...
task.spawn(function()
	local ls = localPlayer2:FindFirstChild("leaderstats") or localPlayer2:WaitForChild("leaderstats", 15)
	if not ls then
		warn("[GalaxyHub] leaderstats never loaded, kill counter disabled")
		return
	end
	local totalKillsVal = ls:FindFirstChild("Total Kills") or ls:WaitForChild("Total Kills", 15)
	if not totalKillsVal then
		warn("[GalaxyHub] Total Kills stat never loaded, kill counter disabled")
		return
	end
	...
	refreshKills()
	totalKillsVal:GetPropertyChangedSignal("Value"):Connect(refreshKills)
	...
	while true do
		...
		-- Keep killsPerHour ticking even between kill events.
		refreshKills()
	end
end)
task.spawn(function()
...
local function setupCombatLoop()
	...
	local function leftClickGoal()
		local myChar = localPlayer2.Character
		local tool = myChar and myChar:FindFirstChildOfClass("Tool")
		return {
			Goal = "LeftClick",
			MousePos = localPlayer2:GetMouse().Hit,
			ToolName = tool and (tool:GetAttribute("Name") or tool.Name),
		}
	end
	local releaseGoal = { Goal = "LeftClickRelease" }
	local function mobileClickGoal()
		local t = leftClickGoal()
		t.Mobile = true
		return t
	end
	local dashGoal = { Dash = Enum.KeyCode.W, Key = Enum.KeyCode.Q, Goal = "KeyPress" }
	local attackAnimIds = {
		"17799224866",
		"17838006839",
		"17838619895",
	...
	local lastSkillCast = { [1] = 0, [3] = 0, [4] = 0 }
	local skillKeycodes = { [1] = Enum.KeyCode.One, [3] = Enum.KeyCode.Three, [4] = Enum.KeyCode.Four }
	local function isSkillSlotReady(slotNumber)
		...
		local slot = hotbarFrame and hotbarFrame:FindFirstChild(tostring(slotNumber))
		local slotBase = slot and slot:FindFirstChild("Base")
		if not slotBase then
			return false
		end
		local cooldown = slotBase:FindFirstChild("Cooldown")
		return cooldown == nil or cooldown.Size.Y.Scale <= 0
	end
	local function tryAutoCastSkills()
		for _, slotNumber in ipairs({ 1, 3, 4 }) do
			if tick() - lastSkillCast[slotNumber] >= 1 and isSkillSlotReady(slotNumber) then
				local keycode = skillKeycodes[slotNumber]
				virtualInput:SendKeyEvent(true, keycode, false, game)
				task.wait(0.001)
				virtualInput:SendKeyEvent(false, keycode, false, game)
				lastSkillCast[slotNumber] = tick()
				task.wait(0.05)
			end
		end
	end
...
then
    if not (targetHumanoid.Health <= 15) then
        if not dashCooldown or tick() - dashCooldown >= 7 then
            communicate:FireServer(dashGoal)
            dashCooldown = tick()
        end
    end
    virtualInput:SendKeyEvent(true, Enum.KeyCode.One, false, game)
    ...
    if not dashCooldown or tick() - dashCooldown >= 3 then
        communicate:FireServer(dashGoal)
        dashCooldown = tick()
    end
    tryAutoCastSkills()
    ...
    if myChar:GetAttribute("mobile") then
        communicate:FireServer(mobileClickGoal())
    else
        communicate:FireServer(leftClickGoal())
    end
    task.wait(0.1)
    communicate:FireServer(releaseGoal)
    task.wait(0.05)
end
		end
		processing = false
